...
from jsplice.lib.jsplicelib import runPreparationStep, runAnalysisStep
...
def main():
	...
	argparser.add_argument('-s','--samestrand',help='Flag for strand-specific RNA-seq for coverageBed (Require same strandedness). Hence not relevant if -b or -j is set.', action='store_true')
	argparser.add_argument('-S','--diffstrand',help='Flag for strand-specific RNA-seq for coverageBed (Require different strandedness). Hence not relevant if -b or -j is set.', action='store_true')
	...
	if args.samestrand and args.diffstrand:
		errmsg += 'ERROR: Select either -s or -S or none but not both!'
	...
	if not os.path.exists(os.path.join(args.outdir,'jSplice.dat')):
		#run the first step
		genome, expDesign, jxnCounts, exnCounts, totCounts, jxn2gene = runPreparationStep(args)
	else:
		#load jSplice object
